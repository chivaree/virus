...
from Rin2.akad.ttypes import Message
from Rin2.akad.ttypes import ContentType as Type
from Rin2.akad.ttypes import ChatRoomAnnouncementContents
from Rin2.akad.ttypes import ChatRoomAnnouncement
...
#                line.acceptGroupInvitation(op.param1)
        ...
        if op.type == 26:
            ...
            if msg.contentType == 16:
                ...
                line.sendMessage(to,'Like Success')
        if op.type == 25:
            msg = op.message
            if msg.contentType == 13:
...
#==============================================================================#
                    ...
                    try:
                        ...
                        if strnum == "off":
                                ...
                                if settings["lang"] == "JP":
                                    line.sendMessage(msg.to,"ปิดใช้งานระบบปฏิเสธคำเชิญตามจำนวนสมาชิก")
                                else:
...
#==============================================================================#
                    ...
                    if msg._from in admin:
                        ...
                        u.write(a)
                        u.close()
                        line.sendMessage(msg.to, "backup has been active")
                        print (wek)
                        print (a)
                        print (r)
                        print (i)
...
#==============================================================================#
                    ...
                    if mic == "on":
                        ...
                        line.sendMessage(msg.to,"👆ค้นหาสำเร็จแล้ว👆")
                        ...
                        line.sendMessage(msg.to,"👆ค้นหาสำเร็จแล้วเชิญกดลิ้งเพื่อเข้าไปดูรายละเอียด👆")
                elif ".คท:" in msg.text:
...
#==============================================================================#
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                               ...
                               mi_d = Mids[:33]
                               line.sendMessage("RED SAMURI Group",mi_d)
                    ...
                    line.sendMessage(msg.to,"welcome to room RED SAMURAI SELFBOT")
  ...
  #                  ke.sendMessage(to, "Ping")         
                    ...
                    if GACReq.responseNum == 0:
                        if msg.toType != 0:
                                ...
                                line.sendMessage(msg._from,"ส่ง SMS สำเร็จแล้ว (｀・ω・´)")
                        ...
                        if msg.toType != 0:
                                ...
                                line.sendMessage(msg._from,"ไม่สามารถส่ง SMS ได้ เนื่องจากมีการส่งข้อความไปยังเบอร์เป้าหมายในเวลาที่ใกล้เคียงกันมากเกินไป (｀・ω・´)\nกรุณารออย่างมาก 30 วินาทีแล้วลองอีกครั้ง")
                    else:
                        if msg.toType != 0:
                                line.sendMessage(msg.to,"พบข้อผิดพลาดที่ไม่รู้จัก (｀・ω・´)")
                        else:
                                line.sendMessage(msg._from,"พบข้อผิดพลาดที่ไม่รู้จัก (｀・ω・´)")
...
#==============================================================================#
                ...
                elif msg.text.lower().startswith(".รูปภาพ "):
                    try:
                        search = text.replace(".รูปภาพ ","")
                        r = requests.get("https://xeonwz.herokuapp.com/images/google.api?q={}".format(search))
                        data = r.text
                        data = json.loads(data)
                        if data["content"] != []:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0 or msg.toType == 1 or msg.toType == 2:
                ...
                if settings ["Api"] == True:
                    if msg.text in ["55","555","5555","55555","55+","555+","5555+","ขำ",".ขำ"]:
                        line.sendMessage(msg.to,"ฮ่าๆๆๆ..ขำไรจ๊ะ..\n😆มึงไปโดนตัวใหนมาเนี่ย😆")
                ...
                if settings["Api"] == True:
                    if msg.text in ["งง","งงง",]:
                        line.sendMessage(msg.to,"ไม่ต้องงงคับ ผมหล่อแบบนี้มาตั้งนานแล้ว😂😂")
            ...
            if settings ["Api"] == True:
                if msg.text in ["สวัดดี","หวัดดี","ดีจ้า","ดีครับ","ดี","สวัสดีครับ","สวัสดีค่ะ","ดีงับๆ","ดีงับ","หวัดดีงับๆ"]:
                    line.sendMessage(msg.to,"🔘▶กองๆเอาใว้ตรงนั้นแหล่ะจ้า อิอิ")
            if settings ["Api"] == True:
                if msg.text in ["/ออน",".ออน",".เวลาออน",".เทส",".อยู่ไหม"]:
                    line.sendMessage(msg.to,"🆗บอททำงานอยู่100%🆗\n🔘[by.™─┅❈͜͡✯WEEBOLINE✯͜͡❈┅─]")
                    line.sendMessage(msg.to,".เวลออน")                    
            if settings ["Api"] == True:
            ...
            if settings ["Api"] == True:
                if msg.text in ["/อ่าน",".คนอ่าน","/คนอ่าน","/ใครอ่าน",".ใครอ่าน"]:
                    line.sendMessage(msg.to,"🔷โปรดดูจำนวลคนแอบซุ่มโจมตี🔷\n🔘[by.™─┅❈͜͡✯WEEBOTLINE✯͜͡❈┅─]")
                    line.sendMessage(msg.to,".อ่าน")
            if settings ["Api"] == True:
                if msg.text in ["เช็คตั้งค่า","เช็คค่า","Set","Status","เช็คค่าเพื่อน"]:
                    line.sendMessage(msg.to,"🔷กำลังเช็คการตั้งค่า🔷.......\n\n🔘[by.™─┅❈͜͡✯WEEBOTLINE✯͜͡❈┅─]")
                    line.sendMessage(msg.to,".เช็ค")
            if settings ["Api"] == True:
                if msg.text in ["ถาม","เรียก","ขานชื่อ"]:
                    line.sendMessage(msg.to,"🆗บอททำงานอยู่100%🆗\n🔘[by.™─┅❈͜͡✯WEEBOTLINE✯͜͡❈┅─]")
                    line.sendMessage(msg.to,"บอทตอบละเด้อ สำบายดีอยู่ดีมีแฮง")
